...
async function sendVipInfo(psid, on){
  return sendText(psid, on ? "VIP enabled—unlimited access. 🚀" : "VIP removed. Standard limits apply.");
}
...
function blank(psid){
  return {
    ...
    followClaimAt: 0,
    // daily cap (verified users)
    dailyCount: 0,
    dailyAt: null, // "YYYY-MM-DD" in Asia/Dhaka
    // profile cache
...
function todayDhaka(){
  return new Intl.DateTimeFormat("en-CA", { timeZone:"Asia/Dhaka", year:"numeric", month:"2-digit", day:"2-digit" })
...
// optional: show name/pic in admin
async function fetchMessengerProfile(psid){
  try{
    if (!PAGE_TOKEN) return null;
    const url = `https://graph.facebook.com/v19.0/${encodeURIComponent(psid)}?fields=first_name,last_name,profile_pic,locale&access_token=${encodeURIComponent(PAGE_TOKEN)}`;
  ...
  try{
    ...
    for (const entry of body.entry || []){
      for (const evt of entry.messaging || []){
        ...
        // first nudge only once + profile cache
        if (!st.verified && !st.nudgeSent){
          ...
          // fallthrough to LLM (counts toward free tier) below
        } else {
        ...
        // Non-verified after free tier
        if (!st.verified && st.freeCount >= FREE_LIMIT){
          if (st.followClaim === "claimed") {
            // every message → pending review; no LLM
            await sendPendingReview(psid);
          } else {
            // user pressed "Not Yet" or never claimed → follow prompt EVERY message; no LLM
            await sendFollowPrompt(psid);
          }
          continue;
        }
        ...
        // Under free tier and not verified yet → normal LLM + increment freeCount
        await markSeen(psid); await sendTyping(psid, true);
        let reply = await generateReplyLLM({ psid, userText: textIn });
        if (reply == null){
          const soft = "Ekto tech jhamela hocche. Abar chesta kortesi, thik hoye jabe 🙂";
